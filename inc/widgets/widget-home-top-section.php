 ...
 * @since      1.0.0
 */
class AdvancedMagazine_Top_Section_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-advancedmagazine-home-top-section',
			'description' => __( 'Display the Featured Slider, Category Block and 160x600 Ad. Only use for the "Homepage" widget area. ', 'advanced-magazine' )
		);

		// Create the widget.
		parent::__construct(
			'happythemes-home-top-section',         // $this->id_base
			__( '&raquo; Home Top Section', 'advanced-magazine' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
			...
			echo '<div class="home-top-section clear">';

				advancedmagazine_home_top_col_one( $args, $instance );

				advancedmagazine_home_top_col_two( $args, $instance );

				advancedmagazine_home_top_col_three( $args, $instance );				

			echo '</div><!-- .home-top-section -->';
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['top_right_ad_code']   = $new_instance['top_right_ad_code'];
	...
	?>
		...
		<h4>Column Three</h4>
		<p>
			<label for="<?php echo $this->get_field_id( 'top_right_ad_code' ); ?>"><?php _e( 'Enter 160x600 Ad Code', 'advanced-magazine' ); ?></label>
			<textarea class="widefat" id="<?php echo $this->get_field_id( 'top_right_ad_code' ); ?>" name="<?php echo $this->get_field_name( 'top_right_ad_code' ); ?>" rows="5"><?php echo( $instance['top_right_ad_code'] ); ?></textarea>
		</p>
 ...
 * Column left posts
 */
function advancedmagazine_home_top_col_one( $args, $instance ) {
	...
	$query = apply_filters( 'advancedmagazine_home_three_columns_1_args', $args );
	...
	if ( $posts->have_posts() ) : 
		...
		require trailingslashit( get_template_directory() ) . 'template-parts/home-top-col1.php';
 ...
 * Column middle posts
 */
function advancedmagazine_home_top_col_two( $args, $instance ) {
	...
	// Allow dev to filter the post arguments.
	$query = apply_filters( 'advancedmagazine_home_three_columns_2_args', $args );
	...
	if ( $posts->have_posts() ) : 
		 ...
		 require trailingslashit( get_template_directory() ) . 'template-parts/home-top-col2.php';
 ...
 * Column right posts
 */
function advancedmagazine_home_top_col_three( $args, $instance ) {

	require trailingslashit( get_template_directory() ) . 'template-parts/home-top-col3.php';
